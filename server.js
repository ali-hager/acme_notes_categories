...
app.post('/api/notes', async(req, res, next)=> {
  try {
    const SQL = `
      ...
      RETURNING *
    `;
    const response = await client.query(SQL, [req.body.text, req.body.category_id]);
...
const init = async () => {
  ...
  SQL = `
    ...
    INSERT INTO categories(name) VALUES ('bazz category');
  ...
  app.listen(port, () => {
    ...
    console.log(`curl -X POST localhost:${port}/api/notes`);
    console.log(`curl -X PUT localhost:${port}/api/notes/1 -d '{"txt":"updated", "category_id": 3, "ranking": 7}' -H "Content-Type:application/json"`);
  });
